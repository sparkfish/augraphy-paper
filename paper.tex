...
To train our model, we developed the ShabbyPages dataset \cite{ref_ShabbyPages}, a real-world document image corpus with \emph{Augraphy}-generated synthetic noise.
Gathering the 600 ground-truth documents was the most challenging part, requiring the efforts of several workers across multiple days. Adding the noise to these images was trivial however; \emph{Augraphy} makes it easy to produce large training sets.
...
Another new dataset --- ShabbyReal --- was produced as part of the larger ShabbyPages corpus \cite{ref_ShabbyPages}; this data was manually produced by applying sequences of physical operations to real paper.  ShabbyReal is fully out-of-distribution for the models in our experiments, and also has a very high degree of diversity, providing good conditions for evaluating \emph{Augraphy}'s effect.
...
\begin{table}
\centering
\caption{Model training statistics and performance on ShabbyReal validation task}\label{tab1}
...
%% \begin{figure}
%% \includegraphics[width=\textwidth]{figures/augmentation-matrix.png}
%% \caption{Groundtruth image (left), sample of \emph{Augraphy} output (right).} \label{fig5}
%% \end{figure}
...
%%     \begin{tabular}{lll}
%%     \toprule
%%     \textbf{Groundtruth String} & \textbf{OCR output on clean image \\ distance to true string} & \textbf{Average OCR output on \emph{Augraphy}-noised images \\ distance to true string} \\
%%     \midrule
...
%% In Table 3 \ref{tab:levenshtein}, it can be seen that \emph{Augraphy} introduces features which confound the Tesseract pre-trained OCR model.
