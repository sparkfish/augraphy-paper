...
\end{center}
\emph{Augraphy} requires only a few other commonly-used Python scientific computing or image handling packages in order to run, such as NumPy \cite{ref_numpy}. \emph{Augraphy} has been tested on Windows, Linux, and Mac computing environments and supports several recent major versions of Python 3. Listing \ref{lst:sample-code} shows how easy it is to get \emph{Augraphy} up and running to create a straightforward augmentation pipeline and apply it to an image:
