...
The \emph{Augraphy} library is designed to produce training datasets, pairing synthetic dirty documents with their clean ground truth document image sources.  The need for such a data augmentation tool in document image analysis primarily results from the lack of ground truth images being available for most of the documents recorded within human history.  Access to the knowledge stored in such document images is thwarted by artifacts, distortion and noise.  As such, there is an ongoing need to improve available techniques to restore document images and thereby unlocking their contents for use in search or other document analysis tasks.
...
Paper documents accumulate physical damage and alterations through printing, scanning, photocopying and daily life.  Real-world processes introduce many types of distortions that result in color changes, shadows in scanned documents, degraded text where low ink, annotations, highlighters or other markings add noise to the page that perturb semantic access by digital means.
...
\begin{figure}
    ...
    \caption{\emph{Augraphy} can be used to introduce noisy perturbations to document images like the original noise seen above, which is a real-life sample from RVL-CDIP. We demonstrate this by creating a noisy reproduction via \emph{Augraphy} augmentations against a clean reproduction.}
...
Table~\ref{tab:augmentation-library-comparison} compares \emph{Augraphy} with other image augmentation libraries and tools.  This analysis highlights that most existing data augmentation libraries do not specifically cater to the corruptions typically encountered in document image analysis datasets.  In contrast, \emph{Augraphy} is designed to be easily integrated into machine learning pipelines alongside popular Python packages used in machine learning and document analysis.  Our paper also utilizes robustness testing to demonstrate how \emph{Augraphy} can be used to evaluate a model's effectiveness in the face of challenging conditions.  Instead of general image modifications, we focus on document-specific distortions and alterations that are better suited to challenge OCR models with document images that still retain sufficient quality to remain human-readable.
...
In contrast, as seen in Table~\ref{tab:augmentation-library-comparison}, only \emph{Augraphy} offers a data augmentation library that supports imitating the corruptions commonly seen in document analysis corpora and is also easily integrated in modern machine learning training processes which rely on Python, including use of specialized paper, ink and post-processing pipelines not seen elsewhere.  In fact, we attempted to recreate the document in Figure~\ref{fig:intro} using \emph{Albumentations} in preparation for presenting such a comparison in this paper, but the results were so poor and unlike the target we aimed to reproduce that we felt our effort would not have been taken seriously.

\emph{DocCreator}~\cite{ref_DocCreator} is the next best option, which provides an image-synthesizing tool for creating synthetic images that mimic common corruptions seen in document collections.  However, \emph{DocCreator} differs from \emph{Augraphy} in several crucial ways.
...
Our paper also uses robustness testing as a way to showcase the effectiveness of \emph{Augraphy}, using specialized document-centric alterations to produce human-readable distortions which challenge OCR and face-detection models.
...
There are four ``main sequence" classes in the \emph{Augraphy} codebase, which together provide the bulk of the library's functionality that when composed generate realistic synthetically-augmented document images:
